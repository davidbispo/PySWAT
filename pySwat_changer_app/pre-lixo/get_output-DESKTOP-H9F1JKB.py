...
class output_rch:
    ...
    def __init__(self,fileaddress,rch_filter,par_filter,observed_input = None, rel_table = None):
...
#Reads personal data 
        if observed_input != None and rel_table != None:
            self.my_data = pd.read_csv(observed_input, delimiter=',',index_col = "GDAY")
...
#Calculates NS from series if in dictionary and in myfiles
                ...
                for i in list(rel_table.values()):
                    ...
                    if temp_array_ef.shape[0] != 0:
                        s = temp_array_ef[:,2]
                        s = s.astype(np.float)
                        o = mydata[rel_table.keys()[rel_table.values().index(i)]].values
                        eff_array = np.stack((s,o)).T
                ...
                for i in keys:
                    ...
                    if key_from_rch in rch_filter:
                        ...
                        if nrows_mydata == nrows_series:
                            plt.plot(temp_array_date, series_array,lw = 1.0, label = r"%s"%(i), color = 'red', alpha = 0.9)
                        else: 
