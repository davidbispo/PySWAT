...
class swat_partable:
 ...
 #gets a filelist for the specificed folder and format 
        ...
        sub_filenumber = len(listtype.listtype(TXTInOut,".sub"))-1 
        #minus 1: output.sub
        ...
        for k in keys:
            ...
            for i in filelist:
                ...
                try :
                    ...
                    print (('Failed to red HRU number on file %s, key %s') %
                           (i, k))
                    exit()
                ...
                SecondHRU_number = firstline_read_single_list[SecondHRU_Start:
                    SecondHRU_End+1]
                ...
                try :
                    ...
                    print (('Failed to convert to float on file %s, key %s' %
                            (i, k)))
                    exit()
                openfile.close()
                if k == keys[0]:
                    ...
                    if i == filelist[0]:
                        par_name = k
                        a = np.array(["LULC","Sub_number","HRU_Number_ABS",
                                      "HRU_Number_REL", par_name]).T
                        new_table = np.array([LULC_type,sub_number,HRU_number,
                                              SecondHRU_number, parameter])
        ...
        if type(sb) == int:
            ...
            for o in sb:
                if len(sb) == 1 and o == sb[0]:
                    ...
                    a_body_filtered = np.vstack((a_body_filtered,
                                                 new_a_body_filtered))
